...
fs.readdir(backgroundsPath, (err, files) => {
  ...
  const backgrounds = files.filter((file) =>
    file.endsWith(`_${qual_imagem}.jpg`)
  );
  fs.readFile(frasesFile, 'utf8', async (err, data) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const fileName = `${i.toString().padStart(5, '0')}_${qual_imagem}.jpg`;
      ...
      context.fillStyle =
        qual_imagem === 'en' ? colorPalette[1] : colorPalette[3];
      ...
      for (let j = 1; j < words.length; j++) {
        ...
        if (lineWidthInFrase < 900) {
          if (wordInFrase.includes('[')) {
            linesInFrase.push(currentLineInFrase);
            currentLineInFrase = wordInFrase.replace('|', '');
          } else {
      ...
      linesInFrase.push(currentLineInFrase);
      const yPosition =
        height / 2 - ((linesInFrase.length - 1) * lineHeight) / 2;
      const xPosition = width / 2;
      linesInFrase.forEach((line, j) => {
        context.fillText(line, xPosition, yPosition + j * lineHeight);
      });
